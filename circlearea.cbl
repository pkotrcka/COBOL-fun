       ...
       01 WS-TRYAGAIN PIC 9(1) VALUE 0.
      ...
      * check if radius is LE than 0, if yes ask for a value
       ...
       RADIUSCHECK.
           IF WS-RADIUS IS <= 0 THEN
                   DISPLAY 'Lower than zero, try again!'
                   GO TO ENTERRADIUS
           ELSE
                   GO TO RADIUSCALC
           END-IF.
      ...
      * asking if user wants to do it again
           DISPLAY 'Do you want to try again? 1 = YES'.
           ACCEPT WS-TRYAGAIN.
      * if his answer is 1, returning to enter radius, otherwise BYE
           IF WS-TRYAGAIN = 1 THEN
                   GO TO ENTERRADIUS
           ELSE
                   DISPLAY FUNCTION TRIM(WS-NAME) WITH NO ADVANCING
                   DISPLAY ', bye then.'
